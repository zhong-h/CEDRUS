...
static void wots_gen_pkx8(unsigned char *pk, const CDS_ctx *ctx,
                 uint32_t addr[8]);
...
int main(void)
{
    ...
    unsigned char wots_pk[8*CDS_WOTS_PK_BYTES];
    ...
    //MEASURE("  - WOTS pk gen 8x.. ", (1 << CDS_TREE_HEIGHT) / 8, wots_gen_pkx8(wots_pk, &ctx, (uint32_t *) addr));
    ...
    //MEASURE("  - WOTS pk gen x8.. ", CDS_D * (1 << CDS_TREE_HEIGHT) / 8, wots_gen_pkx8(wots_pk, &ctx, (uint32_t *) addr));
