...
static void fors_gen_sk(unsigned char *sk, const CDS_ctx *ctx,
...
static void fors_gen_skx8(unsigned char *sk0,
                          ...
                          unsigned char *sk5,
                          unsigned char *sk6,
                          unsigned char *sk7, const CDS_ctx *ctx,
                            ...
                            uint32_t addr_idx, void *info)
{
    ...
    for (j = 0; j < 8; j++) {
        ...
        set_type(fors_leaf_addrx8 + j*8, CDS_ADDR_TYPE_FORSPRF);
    }

    fors_gen_skx8(leaf + 0*CDS_N,
                  leaf + 1*CDS_N,
                  leaf + 2*CDS_N,
                  leaf + 3*CDS_N,
                  leaf + 4*CDS_N,
    ...
    for (j = 0; j < 8; j++) {
        set_type(fors_leaf_addrx8 + j*8, CDS_ADDR_TYPE_FORSTREE);
    }

    fors_sk_to_leafx8(leaf + 0*CDS_N,
                  leaf + 1*CDS_N,
                  leaf + 2*CDS_N,
                  leaf + 3*CDS_N,
                  leaf + 4*CDS_N,
               ...
               const uint32_t fors_addr[8])
{
    ...
    for (i = 0; i < CDS_FORS_TREES; i++) {
        ...
        set_type(fors_tree_addr, CDS_ADDR_TYPE_FORSPRF);
        fors_gen_sk(sig, ctx, fors_tree_addr);
        set_type(fors_tree_addr, CDS_ADDR_TYPE_FORSTREE);
        sig += CDS_N;

        /* Compute the authentication path for this leaf node. */
        treehashx8(roots + i*CDS_N, sig, ctx,
                 indices[i], idx_offset, CDS_FORS_HEIGHT, fors_gen_leafx8,
    ...
    thash(pk, roots, CDS_FORS_TREES, ctx, fors_pk_addr);
}
                      ...
                      const uint32_t fors_addr[8])
{
    ...
    for (i = 0; i < CDS_FORS_TREES; i++) {
        ...
        compute_root(roots + i*CDS_N, leaf, indices[i], idx_offset,
                     sig, CDS_FORS_HEIGHT, ctx, fors_tree_addr);
        sig += CDS_N * CDS_FORS_HEIGHT;
    }
