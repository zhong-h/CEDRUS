                ...
                void *info)
{
    ...
    CDS_VLA(unsigned char, stackx8, 8 * tree_height * CDS_N);
    ...
    for (idx = 0;; idx++) {
        unsigned char current[8*CDS_N];   /* Current logical node */
        ...
        for (h=0;; h++, internal_idx >>= 1, internal_leaf >>= 1) {
            ...
            if (h >= tree_height - 3) {
                if (h == tree_height) {
                    ...
                    memcpy( root, &current[7*CDS_N], CDS_N );
                    return;
                }
            ...
            if ((((internal_idx << 3) ^ internal_leaf) & ~0x7) == 0) {
                memcpy( &auth_path[ h * CDS_N ],
                        &current[(((internal_leaf&7)^1) + prev_left_adj) * CDS_N],
                        CDS_N );
            }
            ...
            unsigned char *left = &stackx8[h * 8 * CDS_N];
            thashx8( &current[0 * CDS_N],
                     &current[1 * CDS_N],
                     &current[2 * CDS_N],
                     &current[3 * CDS_N],
                     &current[4 * CDS_N],
        ...
        memcpy( &stackx8[h * 8 * CDS_N], current, 8 * CDS_N);
    }
}
