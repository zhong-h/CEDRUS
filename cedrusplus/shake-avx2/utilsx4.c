                ...
                void *info)
{
    ...
    CDS_VLA(unsigned char, stackx4, tree_height * 4 * CDS_N);
    ...
    for (idx = 0;; idx++) {
        unsigned char current[4*CDS_N];   /* Current logical node */
        ...
        for (h=0;; h++, internal_idx >>= 1, internal_leaf >>= 1) {
            ...
            if (h >= tree_height - 2) {
                if (h == tree_height) {
                    ...
                    memcpy( root, &current[3*CDS_N], CDS_N );
                    return;
                }
            ...
            if ((((internal_idx << 2) ^ internal_leaf) & ~0x3) == 0) {
                memcpy( &auth_path[ h * CDS_N ],
                        &current[(((internal_leaf&3)^1) + prev_left_adj) * CDS_N],
                        CDS_N );
            }
            ...
            unsigned char *left = &stackx4[h * 4 * CDS_N];
            thashx4( &current[0 * CDS_N],
                     &current[1 * CDS_N],
                     &current[2 * CDS_N],
                     &current[3 * CDS_N],
                     &left   [0 * CDS_N],
        ...
        memcpy( &stackx4[h * 4 * CDS_N], current, 4 * CDS_N);
    }
}
