...
int main(void)
{
    ...
    unsigned char wots_pk[4*CDS_WOTS_PK_BYTES];
    ...
    printf("Parameters: n = %d, h = %d, d = %d, b = %d, k = %d\n",
           CDS_N, CDS_FULL_HEIGHT, CDS_D, CDS_FORS_HEIGHT, CDS_FORS_TREES);

    printf("Running %d iterations.\n", NTESTS);

    MEASURE("Generating keypair.. ", 1, crypto_sign_keypair(pk, sk));
    //MEASURE("  - WOTS pk gen 4x.. ", (1 << CDS_TREE_HEIGHT) / 4, wots_gen_pkx4(wots_pk, &ctx, (uint32_t *) addr));
    ...
    //MEASURE("  - WOTS pk gen x4.. ", CDS_D * (1 << CDS_TREE_HEIGHT) / 4, wots_gen_pkx4(wots_pk, &ctx, (uint32_t *) addr));
