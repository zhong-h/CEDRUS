             ...
             const CDS_ctx *ctx, uint32_t addrx4[4*8])
{
    if (inblocks == 1 || inblocks == 2) {
        ...
        state[CDS_N/8+4] = _mm256_set1_epi64x(0x1f);
        for (int i = CDS_N/8+5; i < 16; i++) {
        ...
        for (unsigned int i = 0; i < (CDS_N/8) * inblocks; i++) {
            state2[CDS_N/8+4+i] = _mm256_xor_si256(
                    state[i],
                    _mm256_set_epi64x(
                        ((int64_t*)in3)[i],
                        ((int64_t*)in2)[i],
         ...
         * of the padding. */
        state2[(CDS_N/8)*(1+inblocks)+4] = _mm256_xor_si256(
            state2[(CDS_N/8)*(1+inblocks)+4],
        ...
        CDS_VLA(unsigned char, buf0, CDS_N + CDS_ADDR_BYTES + inblocks*CDS_N);
        CDS_VLA(unsigned char, buf1, CDS_N + CDS_ADDR_BYTES + inblocks*CDS_N);
        CDS_VLA(unsigned char, buf2, CDS_N + CDS_ADDR_BYTES + inblocks*CDS_N);
        CDS_VLA(unsigned char, buf3, CDS_N + CDS_ADDR_BYTES + inblocks*CDS_N);
        ...
        memcpy(buf0, ctx->pub_seed, CDS_N);
        memcpy(buf1, ctx->pub_seed, CDS_N);
        memcpy(buf2, ctx->pub_seed, CDS_N);
        memcpy(buf3, ctx->pub_seed, CDS_N);
        ...
        shake256x4(bitmask0, bitmask1, bitmask2, bitmask3, inblocks * CDS_N,
                   buf0, buf1, buf2, buf3, CDS_N + CDS_ADDR_BYTES);

        for (i = 0; i < inblocks * CDS_N; i++) {
            buf0[CDS_N + CDS_ADDR_BYTES + i] = in0[i] ^ bitmask0[i];
            buf1[CDS_N + CDS_ADDR_BYTES + i] = in1[i] ^ bitmask1[i];
            buf2[CDS_N + CDS_ADDR_BYTES + i] = in2[i] ^ bitmask2[i];
            buf3[CDS_N + CDS_ADDR_BYTES + i] = in3[i] ^ bitmask3[i];
        }
