          ...
          const unsigned char *in, unsigned long inlen)
{
    ...
    for (i = 0; (i+1)*CDS_SHA256_OUTPUT_BYTES <= outlen; i++) {
        u32_to_bytes(inbuf + inlen, i);
        sha256(out, inbuf, inlen + 4);
        out += CDS_SHA256_OUTPUT_BYTES;
    }
    /* Until we cannot anymore, and we fill the remainder. */
    if (outlen > i*CDS_SHA256_OUTPUT_BYTES) {
        u32_to_bytes(inbuf + inlen, i);
        sha256(outbuf, inbuf, inlen + 4);
        memcpy(out, outbuf, outlen - i*CDS_SHA256_OUTPUT_BYTES);
    }
}
          ...
          const unsigned char *in, unsigned long inlen)
{
    ...
    for (i = 0; (i+1)*CDS_SHA512_OUTPUT_BYTES <= outlen; i++) {
        u32_to_bytes(inbuf + inlen, i);
        sha512(out, inbuf, inlen + 4);
        out += CDS_SHA512_OUTPUT_BYTES;
    }
    /* Until we cannot anymore, and we fill the remainder. */
    if (outlen > i*CDS_SHA512_OUTPUT_BYTES) {
        u32_to_bytes(inbuf + inlen, i);
        sha512(outbuf, inbuf, inlen + 4);
        memcpy(out, outbuf, outlen - i*CDS_SHA512_OUTPUT_BYTES);
    }
}
 ...
 * reused in thash
 **/
void seed_state(CDS_ctx *ctx) {
    uint8_t block[CDS_SHA512_BLOCK_BYTES];
    size_t i;

    for (i = 0; i < CDS_N; ++i) {
        block[i] = ctx->pub_seed[i];
    }
    for (i = CDS_N; i < CDS_SHA512_BLOCK_BYTES; ++i) {
        block[i] = 0;
    }
