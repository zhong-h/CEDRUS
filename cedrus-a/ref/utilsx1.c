                ...
                void *info)
{
    ...
    unsigned char stack[tree_height*CDS_N];

    uint32_t idx;
    uint32_t max_idx = (1 << tree_height) - 1;
    for (idx = 0;; idx++) {
        unsigned char current[2*CDS_N];   /* Current logical node is at */
            /* index[CDS_N].  We do this to minimize the number of copies */
            /* needed during a thash */
        gen_leaf( &current[CDS_N], sk_seed, pub_seed, idx + idx_offset,
                    info );
        ...
        for (h=0;; h++, internal_idx >>= 1, internal_leaf >>= 1) {
            ...
            if (h == tree_height) {
                ...
                memcpy( root, &current[CDS_N], CDS_N );
                return;
            }
            ...
            unsigned char *left = &stack[h * CDS_N];
            memcpy( &current[0], left, CDS_N );
            thash( &current[1 * CDS_N],
                   &current[0 * CDS_N],
        ...
        memcpy( &stack[h * CDS_N], &current[CDS_N], CDS_N);
    }
}
