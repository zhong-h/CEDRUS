...
void wots_gen_leafx1(unsigned char *dest,
    ...
    unsigned int i, k;
    unsigned char pk_buffer[ CDS_WOTS_BYTES ];
    ...
    for (i = 0, buffer = pk_buffer; i < CDS_WOTS_LEN; i++, buffer += CDS_N) {
        ...
        set_type(leaf_addr, CDS_ADDR_TYPE_WOTSPRF);

        prf_addr(buffer, sk_seed, leaf_addr);

        set_type(leaf_addr, CDS_ADDR_TYPE_WOTS);
        ...
        for (k=0;; k++) {
            ...
            if (k == wots_k) {
                memcpy( info->wots_sig + i * CDS_N, buffer, CDS_N );
            }

            /* Check if we hit the top of the chain */
            if (k == wots_w[CDS_WOTS_LEN-1-i] - 1) break;
    ...
    thash(dest, pk_buffer, CDS_WOTS_LEN, pub_seed, pk_addr);
}
