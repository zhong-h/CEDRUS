...
{
    ((unsigned char *)addr)[CDS_OFFSET_LAYER] = layer;
}
...
{
#if (CDS_TREE_HEIGHT * (CDS_D - 1)) > 64
    #error Subtree addressing is currently limited to at most 2^64 trees
#endif
    ull_to_bytes(&((unsigned char *)addr)[CDS_OFFSET_TREE], 8, tree );
}
 ...
 * one of the CDS_ADDR_TYPE constants
 */
void set_type(uint32_t addr[8], uint32_t type)
{
    ((unsigned char *)addr)[CDS_OFFSET_TYPE] = type;
}
...
{
    memcpy( out, in, CDS_OFFSET_TREE+8 );
}
...
{
#if CDS_BOTTOM_TREE_HEIGHT > 8
        /* We have > 256 OTS at the bottom of the Merkle tree; to specify */
        /* which one, we'd need to express it in two bytes */
    ((unsigned char *)addr)[CDS_OFFSET_KP_ADDR2] = keypair >> 8;
#endif
    ((unsigned char *)addr)[CDS_OFFSET_KP_ADDR1] = keypair;
}
...
void copy_keypair_addr(uint32_t out[8], const uint32_t in[8])
{
    memcpy( out, in, CDS_OFFSET_TREE+8 );
#if CDS_BOTTOM_TREE_HEIGHT > 8
    ((unsigned char *)out)[CDS_OFFSET_KP_ADDR2] = ((unsigned char *)in)[CDS_OFFSET_KP_ADDR2];
#endif
    ((unsigned char *)out)[CDS_OFFSET_KP_ADDR1] = ((unsigned char *)in)[CDS_OFFSET_KP_ADDR1];
}
...
{
    ((unsigned char *)addr)[CDS_OFFSET_CHAIN_ADDR] = chain;
}
...
{
    ((unsigned char *)addr)[CDS_OFFSET_HASH_ADDR] = hash;
}
...
{
    ((unsigned char *)addr)[CDS_OFFSET_TREE_HGT] = tree_height;
}
...
{
    u32_to_bytes(&((unsigned char *)addr)[CDS_OFFSET_TREE_INDEX], tree_index );
}
