...
int main()
{
    ...
    for (i = 0; i < CDS_SIGNATURES; i++) {
        printf("  - iteration #%d:\n", i);

        crypto_sign(sm, &smlen, m, CDS_MLEN, sk);

        if (smlen != CDS_BYTES + CDS_MLEN) {
...
#ifdef CDS_TEST_INVALIDSIG
        int j;
        /* Flip one bit per hash; the signature is entirely hashes. */
        for (j = 0; j < (int)(smlen - CDS_MLEN); j += CDS_N) {
            sm[j] ^= 1;
            if (!crypto_sign_open(mout, &mlen, sm, smlen, pk)) {
