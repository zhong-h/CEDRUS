                             ...
                             const unsigned char *seed)
{
    ...
    merkle_gen_root(sk + 3*CDS_N, sk, sk + 2*CDS_N);
                          ...
                          const uint8_t *m, size_t mlen, const uint8_t *sk)
{
    ...
    for (i = 0; i < CDS_D; i++) {
        ...
        merkle_sign(sig, root, sk_seed, pub_seed, wots_addr, tree_addr, idx_leaf,merkle_tree_heights[i]);
                       ...
                       const uint8_t *m, size_t mlen, const uint8_t *pk)
{
    ...
    for (i = 0; i < CDS_D; i++) {
        ...
        thash(leaf, wots_pk, CDS_WOTS_LEN, pub_seed, wots_pk_addr);
