...
#define FORS_COUNTER_OFFSET (CDS_N)
...
void save_wots_counter(uint32_t counter, unsigned char *sig,uint32_t merkle_tree_height) {
    ...
    memcpy(sig + (CDS_WOTS_BYTES + merkle_tree_height * CDS_N), counter_bytes, COUNTER_SIZE);
}
...
uint32_t get_wots_counter(const unsigned char *sig,uint32_t merkle_tree_height) {
    ...
    memcpy(counter_bytes, sig + (CDS_WOTS_BYTES + merkle_tree_height * CDS_N), COUNTER_SIZE);
