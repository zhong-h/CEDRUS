...
static void wots_gen_pkx4(unsigned char *pk, const cds_ctx* ctx,
                uint32_t addr[8]);
...
int main()
{
    ...
    unsigned char pk[CDS_PK_BYTES];
    unsigned char sk[CDS_SK_BYTES];
    unsigned char *m = malloc(CDS_MLEN);
    unsigned char *sm = malloc(CDS_BYTES + CDS_MLEN);
    unsigned char *mout = malloc(CDS_BYTES + CDS_MLEN);

    unsigned char fors_pk[CDS_FORS_PK_BYTES];
    unsigned char fors_m[CDS_FORS_MSG_BYTES];
    unsigned char fors_sig[CDS_FORS_BYTES];
    unsigned char addr[CDS_ADDR_BYTES];
    unsigned char block[CDS_N];

    unsigned char wots_pk[CDS_WOTS_PK_BYTES];
...
#define xstr(s) str(s)
    ...
    randombytes(addr, CDS_ADDR_BYTES);
    #ifdef CDS_FORS_ZERO_LAST_BITS
        fors_bits = CDS_FORS_ZERO_LAST_BITS;
    #endif
    printf("Parameters: n = %d, h = %d, d = %d, b = %d, k = %d, t' = %d, size = %d\n",
           CDS_N, CDS_FULL_HEIGHT, CDS_D, CDS_FORS_HEIGHT, CDS_FORS_TREES, fors_bits, CDS_BYTES);
    sprintf(filename, "%s", xstr(PARAMS));

    fp = fopen(filename,"w+");
    fprintf(fp, "n= %d h = %d d= %d b= %d k= %d tprime= %d size= %d\n",
           CDS_N, CDS_FULL_HEIGHT, CDS_D, CDS_FORS_HEIGHT, CDS_FORS_TREES, fors_bits, CDS_BYTES);
    ...
    //MEASURE("  - WOTS pk gen..    ", (1 << CDS_TREE_HEIGHT), wots_gen_pkx1(wots_pk, &ctx, (uint32_t *) addr));
    MEASURE("Signing..            ", 1, crypto_sign(sm, &smlen, m, CDS_MLEN, sk));
...
static void wots_gen_pkx4(unsigned char *pk, const cds_ctx *ctx, uint32_t addr[8]) {
    struct leaf_info_x4 leaf;
    unsigned steps[ CDS_WOTS_LEN ] = { 0 };
