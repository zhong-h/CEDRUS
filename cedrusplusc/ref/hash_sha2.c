              ...
              const uint32_t addr[8])
{
    ...
    unsigned char buf[CDS_SHA256_ADDR_BYTES + CDS_N];
    ...
    memcpy(buf + CDS_SHA256_ADDR_BYTES, ctx->sk_seed, CDS_N);

    sha256_inc_finalize(outbuf, sha2_state, buf, CDS_SHA256_ADDR_BYTES + CDS_N);
...
{
    (void)ctx;
#define CDS_TREE_BITS (CDS_TREE_HEIGHT * (CDS_D - 1))
#define CDS_TREE_BYTES ((CDS_TREE_BITS + 7) / 8)
#define CDS_LEAF_BITS CDS_TREE_HEIGHT
